...
static var _active_sfx: Dictionary = {}
static func play_sfx_replace(sound: AudioStream, pitch: float = 1.0, volume: float = 1.0, key: String = "") -> void:
	var use_key := key
	if use_key == "":
		if typeof(sound) == TYPE_OBJECT and sound is Resource:
			use_key = (sound as Resource).resource_path
		else:
			use_key = str(sound)

	if _active_sfx.has(use_key):
		var prev: AudioStreamPlayer = _active_sfx[use_key] as AudioStreamPlayer
		if prev != null and prev.is_inside_tree():
			prev.stop()
			prev.queue_free()
		_active_sfx.erase(use_key)
	...
	_active_sfx[use_key] = audio_player

	audio_player.play()
	await audio_player.finished

	if _active_sfx.get(use_key, null) == audio_player:
		_active_sfx.erase(use_key)
	audio_player.queue_free()
...
static func play_sfx_ambient(pos: Vector2, sound: AudioStream, attenuation: float,
	...
	var parent2 := GSceneAdmin.scene_root if GSceneAdmin.scene_root != null else GGameGlobals.instance
	parent2.add_child(audio_player)
...
static func play_music(music: AudioStream) -> void:
	if music_player == null:
		...
		music_player.volume_db = linear_to_db(0.06)
		GGameGlobals.instance.add_child(music_player)

		if music_looped:
			var play_callable := Callable(music_player, "play")
			if not music_player.is_connected("finished", play_callable):
				music_player.connect("finished", play_callable)
...
static func transition_music(new_music: AudioStream) -> void:
	...
	var fade_in_tween = GGameGlobals.instance.create_tween()
	fade_in_tween.tween_property(music_player, "volume_db", org_db, 1.0)
...
class Fx:
	...
	#const SWORD_SWOOSH = preload("res://assets/audio/effects/sword_swoosh.wav")
